...
冲突域按 **每个 m8n8 小块的 8 行** 看:144B 在单块内无起点碰撞;ncu 上 conflict 为 0。
...
## 6. 为什么 wavefront 差很多,cycle 却只涨一截

先钉口径(和 §3 / §5 一致):

- 无冲突基线是 **144B(pad)**,不是 32B。
- wavefront 相对 pad:**32B→2×, 64B→4×, 128B→8×**。题面里说的「约 **4 倍**」对应 **64B**;128B 是 **8 倍** wavefront。
- 8 warp 均摊 cycle 相对 pad:64B 约 **1.15×**,128B 约 **1.7×**。要解释的是:**wavefront 已经按 N 涨,均摊 cycle 却只缓缓抬。**

### 6.1 判断:wavefront 是增量,不是总时间的乘数

**wavefront / bank conflict** 描述的是:同一次 shared 读里,撞 bank 时 Load/Store Unit 要 **多串几拍**。ncu 也是按「实际拍数 / 相对理想多出来的拍数」累加(§5)。

均摊 cycle 更接近:

```text
每次迭代墙钟 ≈ 固定成本 C0 + 冲突相关的排队增量
```

- **C0**:布局无关的固定账——shared 读自身的流水深度、发射/写回、循环、`acc ^= …` 等。pad 换成 128B 时 **C0 几乎不动**。
- **增量**:随 N 路冲突变长的那串拍;叠在 C0 **上面**,不是去乘整个 C0。

#### 收费站直觉

过站本身要一段时间(C0);车多了只是多排队(增量):

- 1 辆车排队 → 总时间 ≈ 固定 + 1 份排队;
- 8 辆车排队 → 总时间 ≈ 固定 + 8 份排队;
- 车数是 8 倍,总时间远不到 8 倍——因为固定通行时间一直在。

#### 示意数字(口径对齐 ncu,量级对齐实测)

下面把「相对 pad 的冲突份数」写成 1 和 8(对应 wavefront **1× / 8×**)。固定成本取 **C0 ≈ 8 cycle**,每份冲突增量约 **1 cycle**(示意,用来看倍数怎么被稀释;不是宣称量到了精确流水线深度):

| 场景 | 冲突份数(相对 pad) | 示意总延迟 | 相对比值 |
|------|-------------------|------------|----------|
| 144B pad | 1 | 8 + 1 = **9** | 1× |
| 128B | 8 | 8 + 8 = **16** | 16/9 ≈ **1.78×** |

实测 8 warp:pad **9.42**、128B **16.00** → **1.70×**,和这张示意表同一量级。

若把循环里的 `i++` / 分支、`acc ^= …` 再单独拆一截固定开销(也算进 C0),只是把「8」拆成「流水几拍 + 算术几拍」,**结构不变**:可变段按 8 倍涨,总时间仍被大头 C0 稀释到约 1.7×。

64B 是 wavefront **4×**:示意 8+4=12 → 约 1.3×;实测只到 10.87(~1.15×),同理——增量还没占满。

> 有人用「10+4=14、10+16=26 → 1.86×」讲同一故事,算术形式一样好。差别只在:我们这里 pad/128 的份数是 **1 与 8**(跟 ncu 的 8192/65536 一致),并把示意总数贴近本题测到的 ~9 / ~16;不宜把 pad 和 32B 都写成 4、再把 128B 写成 16(那是把 8× 收成了 4×)。

### 6.2 8 个 warp 在实验里干什么

源码注释:**8 个 warp 同发把 Load/Store Unit 打满,吞吐由 bank 冲突决定;单 warp 时流水会把串行化掩掉大半。**
...
- **1 warp**:Load/Store Unit 吃不饱,冲突串行化有更多机会被流水叠掉 → 128B 只到 ~1.4× pad。
- **8 warp**(本题):持续喂满通路 → 冲突更容易顶到吞吐 → 128B 到 ~1.7× pad。
- 每个 warp 用 `smem[w * 16 * 160]` 自己的区,访问模式相同。冲突程度由 **单个 warp 内** 行首撞几路 bank 决定;8 个 warp 是 8 条相同压力流,不会把「撞车路数」再 ×8。

本题选 8 warp,是让最差布局在 **cycle** 上露得更清楚,并追问「打满之后 Load/Store Unit 是否已是唯一瓶颈」。即便打满,128B 也只到 ~1.7×——说明 C0 与重叠仍然在,墙钟仍按「固定 + 增量」走,不按 wavefront 乘数走。

### 6.3 和题面数字怎么对上

| 问题 | 答案 |
|------|------|
| 谁相对谁的 4 倍 wavefront? | **64B / pad** |
| 128B 的 wavefront? | **8× pad**(相对 32B 才是 4×,但 32B 自己已有冲突) |
| 为何 cycle 远小于这些倍数? | 冲突代价是叠在固定成本上的 **增量**;再加本题占用下的流水/调度重叠 |
| ncu 看什么 / cycle 看什么? | ncu 验证 N 与 `W=N×W0`;cycle 看增量有多大 |

### 6.4 读数口诀

| 你想证明什么 | 看谁 |
|--------------|------|
| 行跨度 → bank 碰撞路数 | **wavefront 比** 与 **conflict = (N−1)×W0**(pad 为 0) |
| 本题占用下增量有多大 | **cycle**(8 warp;128B ≈ 1.7× pad) |
| 流水把增量藏掉多少 | **对照 1 warp**(128B:12.8 vs 16) |
...
```
无冲突基线 = pad(144B); wavefront: 32→2×, 64→4×, 128→8×
conflicts ≈ wavefronts − W0, W0 = 2×ITERS = 8192
均摊 cycle ≈ 固定成本 C0 + 冲突增量 → 倍数被 C0 稀释(128B≈1.7× pad)
8 warp 打满 Load/Store Unit → 增量更易露头(16 vs 1 warp 的 12.8)
每 warp 独立 smem 区 → 冲突路数由单 warp 内布局决定
```
