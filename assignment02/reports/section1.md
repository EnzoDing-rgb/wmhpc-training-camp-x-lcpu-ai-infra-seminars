...
**wavefront / bank conflict** 描述的是:同一次 shared 读里,因为撞 bank,Load/Store Unit 要 **多串几拍** 才能把请求做完。ncu 里也是按「实际拍数」和「相对理想多出来的拍数」在累加(§5:`conflicts ≈ wavefronts − W0`)。

均摊 cycle 更接近:

```text
每次迭代墙钟 ≈ 固定成本 C0 + 与冲突相关的可变成本
```

- **C0**:这条路径上总要付的账——地址早已算好后的发射、shared 读本身的流水深度、写回、循环开销、`acc ^= …` 防优化等。布局从 pad 换成 128B,**C0 几乎不变**。
- **可变部分**:随 N 路冲突变长的那串 Load/Store Unit 拍。ncu 的 wavefront 把这一段数得很清楚;它叠在 C0 **上面**,不是拿去乘整个 C0。

所以 wavefront 变成 8 倍,意思是「冲突相关的那一截变长了」,不是「整次迭代要跑 8 倍久」。收费站类比:过站固定要一段时间,车多了只是多排队;车数翻倍,总时间涨幅远小于翻倍。

用本题 8 warp 实测估一刀(数量级,不必假装精确拟合流水线深度):
...
wavefront 从 1× 到 8×,墙钟只多了大约 7 拍量级,说明 **可变段相对 C0 并不占满整次迭代**;C0 把倍数稀释掉了。64B(wavefront 4×)只多到 10.87,同理。
